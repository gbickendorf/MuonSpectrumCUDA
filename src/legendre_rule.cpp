#include "/home/gerrit/Documents/Projects/MuonSpectrumCUDA/includes/legendre_rule.h"
...
void float_cgqf(int nt, int kind, double alpha, double beta, double a, double b,
                float t[], float wts[]) {
  double *xt = new double[nt];
  double *wt = new double[nt];
  cgqf(nt, kind, alpha, beta, a, b, xt, wt);
  for (int i = 0; i < nt; i++) {
    t[i] = xt[i];
    wts[i] = wt[i];
  }
...
void cgqf(int nt, double a, double b, double t[], double wts[]) {
  cgqf(nt, 1, 0, 0, a, b, t, wts);
}
